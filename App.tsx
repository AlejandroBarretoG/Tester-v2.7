...
import { ShieldCheck, Server, Database, Settings, XCircle, Code2, ChevronDown, ChevronUp, Bot, Sparkles, KeyRound, Cpu, UserCircle, HelpCircle, Key, HardDrive, TestTube2, CreditCard, Map, Activity, X, Hammer, Smartphone, Layers, LayoutGrid, ArrowLeft, Loader2 } from 'lucide-react';
...
type AppMode = 'firebase' | 'gemini' | 'local' | 'auth_lab' | 'roadmap' | 'voice_lab' | 'modules' | string;
...
const App: React.FC = () => {
  ...
  const getPageTitle = () => {
    ...
    // Check for dynamic module title
    const activeModule = MODULES.find(m => m.id === mode);
    if (activeModule) return activeModule.title;
  ...
  const getPageDesc = () => {
    ...
    // Check for dynamic module description
    const activeModule = MODULES.find(m => m.id === mode);
    if (activeModule) return activeModule.description;
        ...
        <div className="fixed inset-0 z-[90] bg-slate-50/95 backdrop-blur-sm overflow-auto animate-in slide-in-from-bottom-10 fade-in duration-300">
          ...
          <div className="min-h-screen p-6 md:p-12">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="flex justify-center mb-6 overflow-x-auto pb-2">
                  <div className="bg-white p-1 rounded-xl shadow-sm border border-slate-200 inline-flex whitespace-nowrap">
                    ...
                    <button 
                      ...
                      className={`flex items-center gap-2 px-6 py-2 rounded-lg text-sm font-medium transition-all ${
                        mode === 'modules' || MODULES.some(m => m.id === mode) ? 'bg-indigo-100 text-indigo-700' : 'text-slate-500 hover:bg-slate-50'
                      }`}
                    >
                      <LayoutGrid size={18} />
                      Módulos
              ...
              {(mode !== 'local' && mode !== 'auth_lab' && mode !== 'roadmap' && mode !== 'voice_lab' && mode !== 'modules' && !MODULES.some(m => m.id === mode)) && (
              ...
              {mode === 'auth_lab' ? (
                ...
                <ModulesManager onLaunch={(id) => setMode(id)} />
              ) : MODULES.some(m => m.id === mode) ? (
                (() => {
                  const activeMod = MODULES.find(m => m.id === mode)!;
                  return (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                      ...
                      <Suspense fallback={
                        <div className="flex flex-col items-center justify-center py-20">
                          ...
                          <p className="text-slate-500">Cargando módulo {activeMod.title}...</p>
                        </div>
                      }>
                        <activeMod.component />
                      </Suspense>
